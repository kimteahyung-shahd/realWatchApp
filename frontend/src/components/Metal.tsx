...
function Metal (){
    ...
    useEffect(()=>{
        const FetchMetal =async()=>{
            try {
                ...
                let data = link.data;
                let metalData = data.filter((p:any)=>p.type === 'Metal')
                setMetal(metalData);                
    ...
    const handleSort = (e: React.ChangeEvent<HTMLSelectElement>) => {
        ...
        if (value === "low") {
            setMetal([...metal].sort((a, b) => a.price - b.price));
        } else if (value === "high") {
            setMetal([...metal].sort((a, b) => b.price - a.price));
        } else if (value === "Choose"){
            setMetal(originalData); // reset if "choose"
        }
      };    

    return(
    <section>
        <h1 className="text-center font-[600] py-10 text-4xl font-helvetica">Metal</h1>
        <div className="border border-1 py-3 px-20 mb-5">
            ...
            <span>{metal.length} products</span>
            </div>
        </div>
        {metal && (<DisplayProducts Data={metal} />)}
   </section>
)
}
